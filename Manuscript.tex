...
\title[Multisequence Monte Carlo simulations]{Multisequence algorithm for coarse-grained biomolecular simulations: application to protein folding switching}
...
\begin{abstract}
We evaluate a generalized-ensemble algorithm for coarse-grained simulations of biomolecules that allows the thermodynamics of multiple sequences to be calculated in a single ``multisequence" run. By carrying out a random walk in sequence space, conformational sampling is also enhanced. Escape from local energy minima is achieved by visiting sequences for which the minima is shallower or absent. We apply the algorithm alongside a coarse-grained model for protein folding with 7 atoms per amino acid to sets of sequences with up to 35 amino acids spanning a large sequence space. With the multisequence algorithm, we show that the folding thermodynamics of $>$ 1,000 sequences at relatively low temperatures can be readily calculated. We use the resulting data to explore the statistics of pathways in sequence space between two pairs of folds. 
...
\section{Discussion}
\noindent
We have tested an algorithm for coarse-grained biomolecular simulations that relies on making the sequence of the molecule a dynamical parameter. 


For comparison, we used the well-established generalized-ensemble ST algorithm and focused on 144 model sequences with 16 amino acids ({\SR}). As a measure of computational efficiency, we used the statistical error $\sigE$ in the estimation of the average total energy $E$ for 4 different sequences which are part of {\SR}. 


Our results show that at high $T$, well above the folding temperatures of both A1 and N1, we find that the statistical errors $\sigE$ for ST and MS are small and very similar. This is not surprising given that conformational sampling of relatively short polymer chains at high $T$ does not involve crossing major energy barriers. Therefore, successive sampled conformational states, for a given combination of sequence $\sbar$ and temperature $T$, are likely uncorrelated in both ST and MS simulations which, in turn, leads to similar estimates of $\Eave$ (and $\sigE$). At low $T$,...
...
A notable feature of the MS algorithm is that it provides escape from local energy minima on the free energy landscape of proteins without resorting to a change in temperature. This feature could make it attractive as an alternative to exploring the equilibrium behavior of proteins and peptides in lipid membranes, where escape from local minima are especially challenging even in CG simulations.~\cite{Bereau2015} Sampling in membrane simulations at high $T$, such as in replica-exchange simulations, are usually avoided because it can lead to unwanted perturbation of the lipid bilayer structure although it can be a viable approach in particular cases.~\cite{Ulmschneider2010} 

\section{Conclusion}
\noindent
We have evaluated an algorithm for coarse-grained biomolecular simulations that allow the thermodynamics of multiple different sequences to be calculated in a single run. Here we showed that with this MS algorithm, the folding thermodynamics of over 1,000 amino acid sequences with each 35 amino acids could be determined in a coarse-grained model with 3 amino acid types. In addition to being convenient for calculations of multiple sequences, the method enhances conformational sampling with an efficiency comparable to simulated tempering. Random walks across sequence space allow both sampling of many sequences and efficient escape from local free energy minima in the free energy landscape. The method might be well suited for coarse-grained simulations of other biomolecular systems, such as phospholipid bilayers. 
