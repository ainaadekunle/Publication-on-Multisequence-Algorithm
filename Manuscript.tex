...
\begin{abstract}
We evaluate a generalized-ensemble algorithm for coarse-grained simulations of biomolecules that allows the thermodynamics of multiple sequences to be calculated in a single ``multisequence" run. By carrying out a random walk in sequence space, conformational sampling is also enhanced. Escape from local energy minima is achieved by visiting sequences for which the minima is shallower or absent. We apply the algorithm to a coarse-grained model for protein folding with 7 atoms per amino acid and apply it to sets of sequences with up to 35 amino acids spanning sequence space  With the multisequence algorithm, we show that the folding thermodynamics of $>$1,000 sequences at relatively low temperatures can be readily calculated. We use the resulting data to explore the statistics of pathways in sequence space between the two pairs of folds. 
